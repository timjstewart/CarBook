...
public class RatingController {
    ...
    public HttpEntity<Rating> updateOne(@PathVariable UUID carId, @PathVariable UUID userId, @RequestBody Rating resource) {
    ...
    private Rating addLinks(final Rating rating) {
        rating.add(linkTo(methodOn(RatingController.class).getOne(rating.getCar().getUuid(), rating.getUser().getUuid())).withSelfRel());
        return rating;
    }

    private HttpHeaders getCreatedHeaders(final Rating rating) throws URISyntaxException {
        final HttpHeaders responseHeaders = new HttpHeaders();
        final Link location = linkTo(methodOn(RatingController.class).getOne(rating.getCar().getUuid(), rating.getUser().getUuid())).withSelfRel();
